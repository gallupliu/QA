...
class QaLSTMNet(object):
    ...
    def __init__(self, batchSize, unrollSteps, embeddings, embeddingSize, rnnSize, margin):
        ...
        with tf.device("/cpu:0"), tf.name_scope("embedding_layer"):
            ...
            print(tfEmbedding.get_shape())
            print(self.inputQuestions.get_shape())
            print(self.inputTrueAnswers.get_shape())
            print(self.inputFalseAnswers.get_shape())
            #tfEmbedding = tf.Variable(
            #    tf.random_uniform([494493, 50], -1.0, 1.0),
            #    name="W")
