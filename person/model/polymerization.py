...
class LSTM_QA(object):
    def __init__(self, batch_size, num_unroll_steps, embeddings, embedding_size, rnn_size, num_rnn_layers, max_grad_norm, l2_reg_lambda=0.0, adjust_weight=False,label_weight=[],is_training=True):
        ...
        with tf.variable_scope("LSTM_scope", reuse=None):
            ori_q = model_utils.bi_rnn(ori_quests, 'lstm',  self.rnn_size, 1, 1.0, 'bilstm')
            ori_q = tf.concat(ori_q,2)
            ori_q_feat = tf.nn.tanh(max_pooling(ori_q))
        with tf.variable_scope("LSTM_scope", reuse=True):
            cand_a  = model_utils.bi_rnn(cand_quests, 'lstm', self.rnn_size, 1, 1.0, 'bilstm')
            neg_a = model_utils.bi_rnn(neg_quests, 'lstm', self.rnn_size, 1, 1.0, 'bilstm')
            cand_a = tf.concat(cand_a, 2)
            neg_a = tf.concat(neg_a, 2)
            cand_q_feat = tf.nn.tanh(max_pooling(cand_a))
            neg_q_feat = tf.nn.tanh(max_pooling(neg_a))

            test_q_out = model_utils.bi_rnn(test_q, 'lstm', self.rnn_size, 1, 1.0, 'bilstm')
            test_q_out = tf.concat(test_q_out, 2)
            test_q_out = tf.nn.tanh(max_pooling(test_q_out))
            test_a_out = model_utils.bi_rnn(test_a, 'lstm', self.rnn_size, 1, 1.0, 'bilstm')
            test_a_out = tf.concat(test_a_out, 2)
