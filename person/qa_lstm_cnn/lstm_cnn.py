...
class CNN_LSTM(object):
    def __init__(self, batch_size, sequence_len, word_embedding, embedding_size, filter_sizes, num_filters, rnn_size, num_rnn_layers, l2_reg_lambda=0.0):
        ...
        cnn_ori_output = tf.squeeze(tf.concat(axis=3, values=ori_pooled_outputs), [1, 2], name="cnn_ori_output")
        cnn_cand_output = tf.squeeze(tf.concat(axis=3, values=cand_pooled_outputs), [1, 2], name="cnn_cand_output")
        cnn_neg_output = tf.squeeze(tf.concat(axis=3, values=neg_pooled_outputs), [1, 2], name="cnn_neg_output")
