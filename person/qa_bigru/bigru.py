...
def BIGRU_ATT(input_x, rnn_size, batch_size):
    with tf.variable_scope("FW") as fw_scope:
        h_encoder = GRU_ATT(input_x, rnn_size, batch_size, fw_scope)
...
def GRU_ATT(input_x, rnn_size, batch_size, vscope):#input(batch_size, steps, embedding_size)
    ...
    with tf.variable_scope(vscope):
        ...
        for time_step in range(num_steps):
            ...
            h_ = tf.add(tf.mul(u, h_), tf.mul(tf.sub(tf.ones_like(u), u), h))
            output.append(h_)
